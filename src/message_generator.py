class MessageGenerator:
	...
	def generate_Lenkhilfe1(self):
		...
		data = message.encode({
			...
			'Fehlerspeichereintrag__Lenkhilf': 0,
		})
		return message
	...
	def generate_Motor5(self):
		...
		data = message.encode({
			...
			'Kraftstoffverbrauchssignal': 0,
			'Verbrauch_Ueberlauf': 0,
			'K_hlerluefteransteuerung': 0,
			'Klimadrucksignal__Motor_5_': 0,
			'Anlasser_Freigabe': 0,
			'Anlasser_Ausspuren': 0,
			'GRA_Hauptschalter': 0,
			'Doppelte_Momente': 0,
	...
	def generate_Motor1(self):
		...
		data = message.encode({
			'Leergasinformation': 0,
			'Fahrpedalwert_ungenau__Motor_1_': 0,
			'Kickdownschalter': 0,
			'Kupplungsschalter': 0,
			'Time_Out_Bremsen_Botschaft': 0,
			'Fehlerstatus_Brems_Momenteneing': 0,
			'Fehlerstatus_Getriebe_Momentene': 0,
			'Momentenangaben_ungenau': 0,
			'inneres_Motor_Moment': 0,
			'Motordrehzahl': 0,
			'inneres_Motor_Moment_ohne_exter': 0,
			'Fahrpedalwert_oder_Drosselklapp': 0,
			'mechanisches_Motor_Verlustmomen': 0,
			'Fahrerwunschmoment': 0,
		})
		return message
	...
	def generate_Bremse2(self):
		...
		data = message.encode({
			'Timer': 0,
			'Querbeschleunigung': 0,
			'Querbeschl__TimerTic': 0,
			'mittlere_Raddrehzahl__Bremse_2': 0,
			'Zeitstempel': 0,
			...
			'Impulszahl': 0,
			'Frei_Bremse_2_2': 0,
			'gemessene_Querbeschleunigung': 0,
		})
		return message
	...
	def generate_Bremse1(self):
		...
		data = message.encode({
			...
			'Fehlerstatus_Schlechtwegausblen': 0,
			'ABS_in_Diagnose': 0,
			'Aktiver_Bremskraftverstaerker': 0,
			'Geschwindigkeit_neu__Bremse_1_': 0,
			'ASR_Eingriffsmoment_langsam': 0,
			'ASR_Eingriffsmoment_schnell': 0,
			'MSR_Eingriffsmoment': 0,
			'COUNTER': 0,
			'ASR_Steuerger_t': 0,
	...
	def generate_Getriebe2(self):
		...
		data = message.encode({
			...
			'Gradientenbegrenzung': 0,
			'Synchronisations_Wunschdrehzahl': 0,
			'invertierte_Synchronisations_Wu': 0,
			'Synchronisationszeit': 0,
			'Hochschaltlampe': 0,
			'Starter_wird_angesteuert': 0,
			'Gong': 0,
	...
	def generate_Motor2(self):
		...
		data = message.encode({
			...
			'Multiplex_Info_Getriebecode': 0,
			'Multiplex_Info_Max_Moment__Norm': 0,
			'Multiplex_Info_CAN_Stand': 0,
			'Multiplex_Code_Motor_2': 0,
			'Kuehlmitteltemperatur__Motor_2_': 0,
			'Bremslichtschalter': 0,
			'Bremstestschalter': 0,
			'Fehlerstatus_Kuhlmitteltempera': 0,
			'Ansteuerung_Klima__4_1_': 0,
			'Status_Normalbetrieb': 0,
			'OBD_2_freeze_frame': 0,
			'GRA_Status': 0,
			'Fahrzeuggeschwindigkeit': 0,
			'Soll_Geschwindigkeit_bei_GRA_Be': 0,
			'Leerlaufsolldrehzahl__Motor_2_': 0,
			'Begrenzungsmoment': 0,
			'Minimales_Motormoment_bei_Zuend': 0,
		})
		return message